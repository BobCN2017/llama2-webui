...
class PromtsContainer(object):
    ...
    def __init__(self) -> None:
        prompts_path = os.path.join(current_dir,"prompts_zh.csv")
        self.data = read_csv_to_type_dict(prompts_path,"type")
        self.summary_dict = {md5_str(row.get("summary")):row.get("prompt")  for chunk in self.data.values() for row in chunk }
